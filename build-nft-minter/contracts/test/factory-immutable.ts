...
import {
  DynaMintFactory__factory as DyFacFac,
...
describe("Dynamint test", function () {
  ...
  before(async function () {
    ...
    DynaMintNFTFactory = await ethers.getContractFactory("DynaMintFactory");
  });
  beforeEach(async function () {
    dynaMintNft = await DynaMintNFT.deploy();
    await dynaMintNft.deployed();
    dynaMintNftFactory = await DynaMintNFTFactory.deploy(dynaMintNft.address);
